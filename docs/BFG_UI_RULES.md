...
> **Implementation status (rebuild slice 16, 2026-07-13).** The above is the accepted **final** Profile specification. As built today, Slice 16 implements the Profile structure (with real account/training/avatar/subscription data) and the **safe editing** of **Goal** and **Avatar Name** only. The **program-changing** fields (Hero/Heroine direction, Fitness Level, Training Location, Weekly Frequency, Training Structure) currently remain **read-only** pending Program Assignment / Replacement support (D061/D085) and the in-modal D084 confirmation flow. This is an implementation boundary, not a change to the accepted §24 design — all listed fields remain editable in the final target.
