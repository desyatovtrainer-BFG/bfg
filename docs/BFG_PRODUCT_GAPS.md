...
Gap-analysis baseline: 2026-06-12, after the P0A/P0B economy rebalance and data reset. Later branch-scoped implementation updates are dated explicitly in the Summary notes and individual decision entries.
...
- Current status: **Partially Implemented** on `feat/approved-app-rebuild` (2026-07-05, rebuild slice 13) — the old all-caps «…ТВОИ ПОБЕДЫ» marketing screen is replaced by the D074 composition: minimal brand mark → neutral Seed Form in the optical center (temporary static image + breathing/glow, Voice-silent, never a default/final avatar) → calm sentence-case headline + one-line subtitle (placeholder copy per D074 principles) → **single primary CTA «Начать» → /signup** + quiet «Войти» link; no scroll at 360–430px, no trial/pricing/marketing/«победы»/emoji; authenticated `/` still redirects to Home. Auth screens gained the Entry-continuous shell (receded non-interactive Seed silhouette, back → Entry, capped form column). Pending: the D074 **decorative tap-hint** on the Seed Form (tap currently does nothing — still not a navigation affordance, which D074 requires); final copy approval; and the **D076/D077 three-state Auth surface with the blocking OTP gate — Not Implemented** (current auth remains plain Supabase email/password sign-up → Home)
- Current implementation: `app/components/entry/entry-screen.tsx` + `app/page.tsx` + restyled `auth-shell.tsx` on the rebuild branch
- Missing work: rebuild per BFG_UI_RULES §21 / BFG_SCREEN_WIREFRAMES §0 — minimal brand mark; a **Seed Form (First Presence Form)** in the optical center (neutral, unfinished, not gendered, **never a default/final avatar, never Stage 10** — D010; alive but minimal breathing + glow, **Voice-silent**); **Seed Form tap-reactive decoratively only, not a navigation affordance** (decorative path hint toward the CTA, within the §5 motion budget, `prefers-reduced-motion` respected); a **single primary CTA → Sign Up** (the only path forward) with a quiet secondary **Log In** link; calm sentence-case copy (no all-caps, no exclamation, no "победы"/competition framing — D032/§11, no corporate fitness vocabulary, "ты"); **no trial/subscription/pricing** on entry (D006/D030); **no scroll**, mobile-first 360–430px (§1). Onboarding Seed Form behavior is outside D074 scope and is now governed by **D079** (onboarding S1 tap → S2); the D074 Entry Seed Form remains decorative/non-navigational. D074 approves structure, interaction rules, and copy principles only — **no final copy**
...
- Current status: **Partially Implemented** on `feat/approved-app-rebuild` (2026-07-05, rebuild slice 15) — Presence-led S1–S4 built at `/onboarding` (pre-app route, no bottom nav): S1 Seed-Form meeting (finalized 15.1: direct Seed-Form tap → S2 immediately; 2–3s inactivity → dialogue + [Продолжить] → S2; S2–S4 Presence non-interactive; manual browser verification passed) → S2 Goal(multi)+Герой/Героиня → S3 Уровень+Место+условная Частота (матрица D085)+условный Фулбоди/Сплит (только Зал+не-новичок; остальным вопрос не рендерится вовсе) → S4 required Naming (no skip, flag flips only after S4) → full Home (D082, no auto-launch/highlight); resume at earliest unanswered screen (derived from null fields); onboarding grants nothing (D061). Fields persisted via migration 0012 (nullable columns + backfill of existing accounts as completed). Pending: **silent Program Assignment** (D061/D085 — the collected keys are stored but no Program resolves from them yet), progressive avatar-formation art beats (Presence shows seed → direction-at-S4 only), heroine asset, **D077 OTP gate deliberately absent** (signup → onboarding directly — temporary rebuild deviation, insertion point preserved)
- Current implementation: `lib/onboarding/` + `app/onboarding/page.tsx` + `app/components/onboarding/onboarding-flow.tsx` + `(app)/layout.tsx` completion gate + signup redirect
...
- Current status: **Implemented** on `feat/approved-app-rebuild` (rebuild slices 15/15.1, S1 interaction finalized 2026-07-11, manual browser verification passed) — accepted S1–S4 copy and taxonomies/enums in place (S1: direct Seed-Form tap → **S2 immediately**, 2–3s inactivity → dialogue + [Продолжить] → S2, one logical forward action; S2 Goal multi ≥1 with the five enums + «Герой»/«Героиня» labels over `male`/`female`; S3 experience/format labels + conditional frequency refresh-on-change; S4 «Путь выбран. / Осталось выбрать имя.», required name, CTA gating). DB enums CHECK-enforced (0012). Note: Goal-driven avatar starting-form remains Post-MVP as the decision states
- Current implementation: `app/components/onboarding/onboarding-flow.tsx` + `lib/onboarding/actions.ts` validation
- Missing work: **none for D079 itself** — the approved copy, taxonomies/enums, validation, and the finalized S1 interaction (direct tap → S2; inactivity → dialogue + [Продолжить]) are implemented and verified. Remaining onboarding dependencies are tracked under their own decisions, not as D079 work: D077 OTP verification; D061/D085 silent Program Assignment; progressive avatar formation/art
- Recommended priority: closed — no open P1 implementation task for D079

### D080 — Editable Onboarding Inputs After Onboarding
- Current status: **Partially Implemented** on `feat/approved-app-rebuild` (rebuild slice 16, 2026-07-13)
- Current implementation: Profile edits the two **safe** inputs — **Goal** (multi-select, ≥1) and **Avatar Name** (global, D083) — via authenticated Server Actions (`lib/profile/actions.ts`), immediate save with no progression reset, wired into the D086 Profile screen. Goal/Name changes never alter Program Assignment. The other editable inputs (Hero/Heroine, Experience, Training Format, Weekly Frequency, Training Structure) are **displayed read-only** and not yet editable
- Missing work: editing of **Hero/Heroine, Experience, Training Format, Weekly Frequency, Training Structure** (preliminarily in Profile) with **no reset** of XP/Level/Stage/Streak/Workout History/Weight History/Avatar Progression; Sex/Experience/Format/Frequency/Structure changes trigger Program Replacement / recalculation (D061 §7, D085)
- Dependency / boundary: these are **program-changing** edits deferred until safe Program Assignment / Program Replacement support exists (D061/D085) with the D084 confirmation flow and D058 active-workout deferral. The accepted D080 scope is **not** reduced — all listed inputs remain editable in the final design; only the safe subset is wired now
- Recommended priority: P1 — pairs with the D079 taxonomy and the Profile surface (D006); remaining fields pair with the Program Assignment build
...
- Current status: **Implemented (MVP scope)** on `feat/approved-app-rebuild` (rebuild slices 10/15) — after S4 the user lands on the full Home with **no auto-transition to Workout 1 and no first-run CTA highlight**; the **Avatar Name renders as the first identity line of the Stage Block** (Living Presence → Имя → Стадия → Voice Slot → Continue Journey), never in the header/CTA/stat card; starting values framed calmly. Brand-new user's Continue Journey still resolves to Workout 1 (D059/D043, via the journey resolver)
- Current implementation: `home-screen.tsx` Stage Block + `dashboard/page.tsx` (real name/direction) + onboarding S4 → `/dashboard` handoff
...
- Current status: **Partially Implemented** on `feat/approved-app-rebuild` (rebuild slice 16, 2026-07-13)
- Current implementation: the accepted **safe-field** flow is built for **Goal** and **Avatar Name** — a single Profile modal container, immediate save, user stays on Profile, calm inline success, inline error, no toast, all via authenticated Server Actions (identity from the session, no client `user_id`)
- Missing work: the program-changing **confirmation flow** — a detailed **D083-accurate Hero/Heroine modal** and a shorter modal for the training fields (Experience, Training Format, Weekly Frequency, Training Structure), using the accepted Russian copy in the D084 registry entry; the **In-Progress-workout deferral** line/behavior (active workout unchanged, new assignment after it finishes — **D058/D061**); the auto-resolve-to-Full-Body-inside-the-same-flow rule when a change disallows Split (D085); any pending-assignment state
- Dependency / boundary: the confirmation flow waits for Program Assignment / Program Replacement support (D061/D085). Does not decide Profile layout, DB schema, customization catalog, Nutrition logic, or payment
...
- Current status: **Not Implemented** — rebuild slice 16 adds **Profile read-side presentation only** (no assignment logic)
- Current implementation: D061 assignment is Sex × Fitness Level × Training Format only; the old D081 weekly-frequency matrix is unimplemented and now replaced; no Training Structure field; no conditional Full Body/Split choice; no reduced 8-Program model; no Full Body active-subset logic; no separate 3-/4-day Split Programs; no Profile editability for Training Structure; no assignment resolver for the D085 rules. Slice 16 only **displays** the stored structure with the accepted Russian labels (Фулбоди/Сплит) and a **display-only effective Full Body inference** for legacy null values (shown as Фулбоди when Training Format = home, Fitness Level = beginner, or Weekly Frequency = 2) — this reads existing data and performs **no database write** and **no** assignment
...
- Current status: **Partially Implemented** on `feat/approved-app-rebuild` (rebuild slice 16, 2026-07-13)
- Current implementation: the final Profile structure is built and wired to real data — section hierarchy **Аккаунт → Тренировки → Аватар → Подписка → «Выйти»** (no «Система»), real account email, real training values (Цель · Уровень · Место тренировок · Тренировок в неделю · Формат тренировок), real avatar values (Имя · Направление), real subscription state, **Goal** and **Avatar Name** editing via the single-modal flow (`profile-goal-row.tsx` / `profile-name-row.tsx` + `app-modal.tsx`), the existing logout confirmation modal, and read-only treatment (no chevron/modal/tap) for the program-changing rows; legacy null values show «Не указано»
- Missing work: editing of **Hero/Heroine, Fitness Level, Training Location, Weekly Frequency, Training Structure**; the in-modal program-change confirmation states (D084); and Program Assignment / Replacement integration
- Dependency / boundary: the remaining edits and the in-modal confirmation states depend on Program Assignment / Replacement support (D061/D085) and the D084 confirmation flow. Build per `BFG_UI_RULES.md §24` / `BFG_SCREEN_WIREFRAMES §10` — administrative Profile titled **«Профиль»** (not «Настройки»; subtitle «Данные, которые помогают подобрать тренировки и сохранить твой путь.»), structure **Аккаунт → Тренировки → Аватар → Подписка → «Выйти»** (no «Система» section); **Аккаунт** email read-only (no verification status on the happy path); **Тренировки** rows Цель (safe, immediate, Nutrition-only later) / Уровень / Место тренировок / Тренировок в неделю / Формат тренировок (program-changing via D084; frequency options and Формат-editability gated by D085 — editable only for Gym Int 3 / Gym Adv 3 / Gym Adv 4, else read-only auto-Full-Body); **Аватар** rows Имя (safe, global) / Направление (detailed D084 Hero/Heroine confirm + D083 direction-slot behavior); **Подписка** read-only calm state (payments post-MVP); **«Выйти»** standalone → logout confirmation modal (deletes/resets nothing → Entry/Auth). **Single-modal edit flow:** one Profile modal at a time, selection and D084 confirmation as states of the same modal (no stacked modals, no separate edit screens, no toast), calm inline success after save; read-only rows have no chevron. Profile does not open Avatar Customization (Home only, D073), shows no app settings, and shows no Progress/Home/stats/history content
