...
### D081 — Conditional Weekly Frequency and Program Family Model
- Current status: Not Implemented
...
- Missing work: add `weekly_frequency` as a fourth assignment dimension and a **Program Family / Program Variant** model (internal-only) — shared scalable 4-workout **Home** family (frequency = active workout count, 2/3/4) vs frequency-specific authored **Gym** variants (beginner/intermediate gym 2–3/wk full-body; advanced gym 3–4/wk with separate 3- and 4-day split variants, never truncated); conditional S3 frequency UI (hidden until Experience + Format; allowed-subset only; clear-on-change); edit-triggers-replacement-without-reset; avoid 24 independent monthly programs without faking gym scalability. Documented in `fitness/BFG_PROGRAM_ARCHITECTURE.md`
- Recommended priority: P1 — refines D061; required for correct silent assignment after S3
