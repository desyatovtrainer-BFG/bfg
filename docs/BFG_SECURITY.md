...
- ❌ The client never sends an XP amount. Server Actions accept either a typed `XpRewardSource` from a closed enum (`WORKOUT_COMPLETE`; `MILESTONE` is reserved and unwired) or a server-resolved amount from the quest catalog — the client submits only a quest id. Daily quest claims are additionally validated against the server-recomputed daily selection (D017) before any DB access. Streaks never grant XP and daily login XP is out of MVP — the former constants were removed; see [`BFG_GAME_SYSTEMS.md`](./BFG_GAME_SYSTEMS.md) §2.2.
