...
1. **Sign up** with email + password, **verify the email with a one-time code**, then complete onboarding. The MVP auth/onboarding path is **Sign Up → Verify Email → Onboarding → Home** (Decisions 074, 076, 077). Onboarding (Decision 078) is a **Presence-led dialogue** of four screens — **S1 Seed Form → S2 Goal + Sex → S3 Fitness Level + Environment/Home-Gym → S4 Default Avatar + Naming Ceremony** — with a silent, deterministic Program Assignment (Sex × Fitness Level × Training Format, D061) between S3 and S4. Email verification is a **required, hard-blocking** pre-onboarding OTP gate (D077); the Naming Ceremony is required (D078). Option taxonomies, final copy, and final avatar art direction are not locked.
...
- **Onboarding flow (Decision 078)** — a **Presence-led dialogue** (the Presence is on every screen and owns the question framing; options are the user's structured replies — not a form wizard), four screens: **S1 Seed Form → S2 Goal + Sex → S3 Fitness Level + Environment/Home-Gym → S4 Default Avatar + Naming Ceremony**, then Home. The Stage-1 default avatar first appears on S4 (never Stage 10 — D010); **Sex** drives Program Assignment and the basic default-avatar direction, **Goal** is collected for framing only (not assignment-driving), **Naming is required**. Program Assignment is silent and deterministic (Sex × Fitness Level × Training Format, D061; no screen). No verification/legal/payment field on any onboarding screen (D077). See `BFG_UI_RULES.md §23` / `ui/BFG_SCREEN_WIREFRAMES.md §0.2`. Option taxonomies, final copy, and final avatar art direction are not locked.
