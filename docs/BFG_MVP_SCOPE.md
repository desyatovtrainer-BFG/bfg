...
1. **Sign up** with email + password, **verify the email with a one-time code**, then complete onboarding. The MVP auth/onboarding path is **Sign Up → Verify Email → Onboarding → Home** (Decisions 074, 076, 077). Onboarding (Decisions 078–085) is a **Presence-led dialogue** of four screens — **S1 Seed Form → S2 Goal + Hero/Heroine → S3 Experience + Training Format + Weekly Frequency + Conditional Training Structure → S4 Default Avatar + Naming Ceremony** — with a silent, deterministic Program Assignment (**avatar direction × Training Format × Training Structure × Weekly Frequency**, D061/D085) between S3 and S4, where **Experience gates the allowed Weekly Frequency and Training Structure options rather than multiplying Program Families** (D085). Email verification is a **required, hard-blocking** pre-onboarding OTP gate (D077); the Naming Ceremony is **required** (D078/D079). MVP collects **Goal, Hero/Heroine, Experience, Training Format, Weekly Frequency, Training Structure, and Avatar Name**, all **editable later in Profile** with no progression reset (D080) and with the save/confirmation behavior of D084. Goal does **not** drive training Program Assignment (it may later affect **Nutrition** recommendations only). The reduced D085 model is **8 coach-authored Programs / 30 Workout Templates**. Screen copy and answer taxonomies are accepted (D079); **final avatar art direction and Goal-driven avatar starting-form variation are Post-MVP**.
...
- **Onboarding flow (Decisions 078–081, 084, 085)** — a **Presence-led dialogue** (the Presence is on every screen and owns the question framing; options are the user's structured replies — not a form wizard), four screens: **S1 Seed Form → S2 Goal + Hero/Heroine → S3 Experience + Training Format + Weekly Frequency + Conditional Training Structure → S4 Default Avatar + Naming Ceremony**, then Home. Collects **Goal (multi-select, framing-only — not training-assignment-driving; may later feed Nutrition only), Hero/Heroine (`male`/`female`), Experience (`beginner`/`intermediate`/`advanced`), Training Format (`home`/`gym`), Weekly Frequency (`two`/`three`/`four_per_week`, allowed set by Experience — **Beginner 2/3 · Intermediate 3 · Advanced 3/4**, same for Home and Gym, D085), Training Structure (`full_body`/`split`, shown only for **Gym + non-Beginner** — Gym Int 3 / Gym Adv 3 / Gym Adv 4 — else auto Full Body, D085), and Avatar Name (required)**. The Stage-1 default avatar first appears on S4 (never Stage 10 — D010), direction from Hero/Heroine. **Program Assignment is silent and deterministic** (**avatar direction × Training Format × Training Structure × Weekly Frequency**; Experience only **gates** the allowed frequency/structure options and no longer multiplies families — D085/D061; no screen). All inputs are **editable later in Profile with no progression reset (D080)**, with save/confirmation behavior per **D084** (Goal & Avatar Name save immediately; the program-changing fields use a calm confirmation modal; an In-Progress workout defers the new assignment until it finishes — D058/D061). No verification/legal/payment field on any onboarding screen (D077). Screen copy and taxonomies are accepted (D079). See `BFG_UI_RULES.md §23` / `ui/BFG_SCREEN_WIREFRAMES.md §0.2`. **Reduced Program Family model (D085):** exactly **8 coach-authored Programs / 30 Workout Templates** — Home Full Body Hero/Heroine (4+4), Gym Full Body Hero/Heroine (4+4), Gym 3-Day Split Hero/Heroine (3+3), Gym 4-Day Split Hero/Heroine (4+4). **Home is always Full Body; Split is Gym-only and never for Beginner; 2 days always resolves to Full Body.** Full Body Programs expose a **frequency-sized active subset** (2→first 2, 3→first 3, 4→all 4); Split Programs are frequency-specific and never truncated (3-day Split shared by Intermediate/Advanced, 4-day Split Advanced-only). Changing assignment grants no XP and resets no progress; Program Replacement / active-workout snapshot behavior is unchanged (D058/D061 §7). **First Home after onboarding (D082):** after S4 the user lands on the **full Home, not Workout 1** — no auto-launch of the first workout and no first-run highlight on the "Continue Journey" CTA. The **Avatar Name appears on Home** (first identity line of the Stage Block), and **avatar customization may be entered from the Home Living Presence before the first workout** (D073); the user starts the first workout only when they choose to (for a brand-new user, Continue Journey resolves to Workout 1 — D059/D043). **Post-MVP / future:** final avatar art direction and Goal-driven avatar starting-form variation; Goal-driven Nutrition recommendations.
...
> **Progress & avatar identity (Decisions 072, 073).** The Progress surface (the merged Avatar + Progress + Profile destination, D005) is structured per D072: a **static, non-interactive identity portrait** (Primary) → Level / XP / Streak / Stage position (Secondary) → History / Statistics / Achievements as entry points (Additional). Avatar customization is entered **only from the Home Living Presence** (tap → Avatar Customization, D073); Progress never opens customization. **MVP correctness:** there is **one shared avatar visual state**, and the Home live Presence and the Progress static portrait must always render the **same** customized avatar — Home↔Progress visual parity is a correctness property (D001 at the rendering layer), not deferrable polish. **Avatar direction slots (Decision 083, MVP):** because Hero/Heroine is editable later in Profile (D080), the shared avatar state is **direction-specific** — separate Hero and Heroine visual slots. Changing Hero/Heroine switches the **active direction only**, with **no customization migration** between directions; the new direction renders its **saved slot** if present, otherwise its **default avatar at the current global Stage** (default avatars exist per direction at every Stage 1–10, D010; never Stage 1 for a mid-journey user). The other direction's slot is **preserved, not deleted**; Avatar Name and Stage stay **global**; the change resets no progression; and it stays **one Presence** (D001). D083 fixes persistence/switching only — not customization catalog depth (still scoped separately per §3.2), art, paid cosmetics, currency, or DB schema. **Not forced into MVP by these decisions:** the customization **catalog depth** (clothing/cosmetics catalogs, currency-linked cosmetics, rich editor) — the *entry model* and *shared state* are MVP; *catalog depth* is scoped separately and may be MVP or Post-MVP (cosmetics remain catalog-only at MVP per §3.2, and never affect gameplay — D029/D034).
