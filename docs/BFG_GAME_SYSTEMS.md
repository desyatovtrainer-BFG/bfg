...
- A superset group must contain **exactly 2 consecutive exercises** sharing the same `superset_group_id` within a workout. Groups with 1 or 3+ members, or non-adjacent members, are a content authoring error — the session treats affected exercises as normal slides and `listWorkoutExercises` emits a server-side `console.warn` for each violation.
