...
- Implementation Status is updated incrementally. The registry baseline was 2026-06-12 after the P0A/P0B economy rebalance and data reset; later branch-scoped implementation updates are dated explicitly in the relevant decision entries and reflected in the Implementation summary.
...
2. Seed Form interaction is context-scoped (no global clickable / non-clickable
   ...
   - Onboarding Seed Form behavior is outside D074 scope. (Boundary updated
     2026-07-11: it is now governed by Decision 079 — the onboarding S1 Seed
     Form is functional and advances to S2, while the Entry Seed Form here
     remains decorative and non-navigational; S2–S4 Presence is
     non-interactive. The two interactions are context-scoped and
     intentionally different. D074 still governs Entry only and neither
     defines nor overrides D079.)
...
Decision 079 since 2026-07-11 — the S1 Seed Form tap advances to S2; the Entry Seed Form stays decorative.)
...
1. S1 — Seed Form / First Meeting.
   ...
   - Direct action: tapping the S1 Seed Form immediately advances to S2 — the
     tap does NOT reveal the dialogue first, requires no second tap, and uses
     the same internal S1 → S2 transition as [Продолжить].
   - Inactivity path: after 2–3 seconds without interaction, the Presence says
     "Давай сделаем первый шаг. / Я помогу тебе начать." with CTA [Продолжить];
     pressing [Продолжить] advances to S2.
   - After the dialogue appears, tapping the Seed Form still advances to S2 —
     the Seed Form and [Продолжить] are the same logical forward action,
     leading only to S2.
   - Only the S1 Presence is interactive; the Presence on S2–S4 is
     non-interactive and non-focusable. (The Entry-screen Seed Form remains
     decorative and non-navigational — D074; this interaction is
     onboarding-S1-scoped only.)

   Update note (2026-07-11, S1 interaction finalization): this finalizes the
   S1 interaction and supersedes the earlier "tap OR inactivity reveals the
   dialogue" wording — a direct Seed Form tap now advances to S2 immediately;
   the reveal belongs to the inactivity path only. The accepted S1 copy is
   unchanged.
...
Implementation Status:
Implemented on `feat/approved-app-rebuild` (rebuild slices 15/15.1, 2026-07-11) —
S1–S4 copy, taxonomies/enums, validation, and the finalized S1 interaction
(direct Seed Form tap → S2; inactivity → dialogue + [Продолжить]) are built and
manually verified. Dependent items remain pending under their own decisions:
D077 OTP gate, silent Program Assignment (D061/D085), progressive avatar
formation/art.
...
Implementation Status:
Partially Implemented on `feat/approved-app-rebuild` (rebuild slice 16, 2026-07-13) —
Profile now edits the two **safe** onboarding inputs: **Goal** (multi-select, ≥1) and
**Avatar Name** (global, D083), both via authenticated Server Actions with immediate
save and calm inline success (the D084 safe-field flow). The remaining editable inputs —
**Hero/Heroine, Experience (Fitness Level), Training Format, Weekly Frequency, Training
Structure** — are **displayed read-only** and are **not yet editable**: they are
program-changing and their edits are **deferred until safe Program Assignment / Program
Replacement support exists** (D061/D085, D084 confirmation + D058 active-workout
deferral). This does **not** reduce the accepted D080 scope — all listed inputs remain
editable in the final design; only the safe subset is wired in this slice.
...
Implementation Status:
Partially Implemented on `feat/approved-app-rebuild` (rebuild slice 16, 2026-07-13) —
the accepted **safe-field** flow is built for **Goal** and **Avatar Name**: a single
Profile modal container, immediate save, the user stays on Profile, calm inline success,
inline error, no toast, all via authenticated Server Actions (identity from the session,
no client `user_id`). **Still missing:** the program-changing **confirmation flow**
(Hero/Heroine detailed modal + the shorter training modal), Program Replacement, the
active-workout deferral line/behavior (D058/D061), and any pending-assignment state —
those wait for Program Assignment / Replacement support.
...
Implementation Status:
Not Implemented. (Rebuild slice 16 adds only **read-side** Profile presentation of the
training-structure value — the accepted Russian labels (Фулбоди/Сплит) and a
**display-only effective Full Body inference** for legacy null values (shown as Фулбоди
when Training Format = home, Fitness Level = beginner, or Weekly Frequency = 2), which
performs **no** database write. This does **not** implement the reduced Program model,
Program Assignment, Program Families/Variants, or Program Replacement. D085 stays
**Not Implemented**.)
...
Implementation Status:
Partially Implemented on `feat/approved-app-rebuild` (rebuild slice 16, 2026-07-13) —
the final Profile screen structure is built and wired to real data: the section
hierarchy (Аккаунт → Тренировки → Аватар → Подписка → «Выйти», no «Система»), real
account email, real training values (Цель · Уровень · Место тренировок · Тренировок в
неделю · Формат тренировок), real avatar values (Имя · Направление), real subscription
state, **Goal** and **Avatar Name** editing via the single-modal flow, the existing
logout confirmation modal, and read-only treatment (no chevron/modal/tap) for the
program-changing rows. **Still missing:** editing of **Hero/Heroine, Fitness Level,
Training Location, Weekly Frequency, Training Structure**, the in-modal program-change
confirmation states (D084), and Program Assignment / Replacement integration.
...
|---|---|---|
| Implemented | 18 | 001, 010, 011, 012, 013, 015, 017, 018, 019, 020, 021, 023, 029, 030, 031, 032, 033, 079 |
| Partially Implemented | 13 | 002, 007, 009, 016, 022, 035, 036, 037, 038, 075, 080, 084, 086 |
| Not Implemented | 55 | 003–006, 008, 014, 024–028, 034, 039–074, 076, 077, 078, 081, 082, 083, 085 |
...
Decision 074 (Entry / Auth Start Screen) was accepted 2026-06-26. It is the first decision covering the **unauthenticated** surface (all prior screen decisions — D039/D042–D069/D071/D072 — are post-auth) and introduces no contradiction: it honors no-Stage-10-early (D010), the single-primary-CTA discipline (D071, D073), no-shame / no-competition framing (D031, D032, "победы" forbidden per BFG_UI_RULES §11), the core-loop-never-gated rule (D030, no trial/pricing on entry), and the Body-never-reaches / under-promise onboarding philosophy (Companion Doctrine §X/§XI). It deliberately keeps the **Seed Form interaction context-scoped** — "On the Entry / Auth Start screen, Seed Form is tap-reactive decoratively but is not a navigation affordance" — and records **no global** clickable / non-clickable rule. D074 is **self-contained**: at acceptance time the first-onboarding Seed Form interaction was governed separately by future onboarding decisions ("none accepted today" was true then; **since 2026-07-11 it is governed by Decision 079** — the onboarding S1 Seed Form tap advances to S2, while the Entry Seed Form remains decorative), so D074 forward-referenced no accepted onboarding decision and invented none. D074 approves **structure, interaction rules, and copy principles only — not final copy** (current headline/subtitle/CTA variants remain placeholders). One UI follow-up, not a contradiction: the decorative tap-hint must stay within the `BFG_UI_RULES.md §5` motion budget (up to ~600ms, prefers-reduced-motion respected) — recorded for the §5/§13 pass. Recorded in `BFG_UI_RULES.md` §21 (new Entry section) and `docs/ui/BFG_SCREEN_WIREFRAMES.md` (new Entry / Auth Start section), with a one-line precedence note added to `BFG_MVP_SCOPE.md §2.1`. Not Implemented. Total decisions: **74**.
